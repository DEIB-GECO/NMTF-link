...
for normalization in normalizations:
    for mask in masks:
        for integ_strategy in integ_strategies:
            for strategy in init_strategies:
                for metric in metrics:
                    rel_error = []
                    iterations = []
                    for n in tqdm(n_iter):
                        ...
                        with open('case_study_4/graph_topology_config.tsv', 'w') as f:
                            f.write("#parameters\n"
                                    f"#integration.strategy	{integ_strategy}\n"
                    ...
                    df1 = pd.DataFrame([iterations], columns=n_iter).melt()
                    ...
                    #sns.lineplot(x='variable', y='value', data=df1, ci='sd', label=mask + '_' + strategy + '_' + metric, ax=ax2)
