...
class AssociationMatrix():
    def __init__(self, filename, leftds, rightds, left_sorted_terms, right_sorted_terms, main, mask, type_of_masking, verbose):
    ...
    def compute_tf_idf2(self):
        self.tf_rows=np.true_divide(np.count_nonzero(self.original_matrix, axis=1), np.count_nonzero(self.original_matrix)) #axis - Axis or tuple of axes along which to count non-zeros. 
        self.tf_columns=np.true_divide(np.count_nonzero(self.original_matrix, axis=0),np.count_nonzero(self.original_matrix))

    def compute_tf_idf(self):
        self.tf=np.true_divide(self.association_matrix, np.repeat(np.count_nonzero(self.association_matrix, axis=1)[:,np.newaxis], self.association_matrix.shape[1], axis=1) +0.00000001 )
        #axis - Axis or tuple of xes along which to count non-zeros. 
        self.idf=np.repeat(np.log(np.true_divide(np.count_nonzero(self.association_matrix), np.count_nonzero(self.association_matrix, axis=0) + 0.00000001 ))[np.newaxis, :], self.association_matrix.shape[0], axis=0)
